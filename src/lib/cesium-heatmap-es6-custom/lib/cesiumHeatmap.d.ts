...
export declare class CesiumHeatmap {
  ...
  /**
   * 把经纬度转换成屏幕坐标
   * @param newPoints 类型是HeatmapPoint
   * @return 返回canvas屏幕坐标
   */
  lnglat2canvas(newPoints: HeatmapPoint[]): [];
  /**
   * @param newData heatmap.setData({max,min,data})
   * data是把经纬度转换成屏幕坐标后的数组 [{x,y,value},{},{},...]
   * 这个函数可以更新热力图样式
   */
  updateCanvasDataOnly(newData): void;
  /**
     * 更新热度图配置
     * @param options
     */
  updateHeatmap(options): void
  private createLayer;
