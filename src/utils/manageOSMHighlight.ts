...
export function createHighlightManager(tileset: Cesium.Cesium3DTileset) {
  ...
  return {
    ...
    setCategoryIds(entityId: string, categoryIds: Record<string, number[]>) {
      ...
      // console.log('highlightedIds[entityId]', highlightedIds[entityId]);
      Object.keys(categoryIds).forEach(key => {
        highlightedIds[entityId][key] = categoryIds[key]
        // console.log(highlightedIds[entityId][key])
      })
      //改成全局刷新 不只是刷新当前的圆
      updateStyle()
    },
    ...
    removeCategoryIds(entity: string) {
      highlightedIds[entity] = {}
      updateStyle()
    },

    /**
     * 得到当前高亮建筑的id
     * @param category 可选，获得类型的id，默认获取全部
     */
    // getAllIds(category?: keyof typeof highlightedIds) {
    // if (category) return [...highlightedIds[category]]
    // 下面这句什么意思？ 
    /**    highlightedIds 是一个对象
         * {
            commercial: Set(1,2,3),
            accommodation: Set(4,5),
            civic: Set(6),
            transportation: Set(7,8)
          }
          Object.values 会返回对象的值数组，也就是
          [ Set(1,2,3), Set(4,5), Set(6), Set(7,8) ]
          
          .flatMap((s) => [...s])
          s 是每一个 Set<number>。
          [...s] 把 Set 转成数组，例如 Set(1,2,3) → [1,2,3]。
          flatMap 会把所有数组“扁平化”成一个单一数组，而不是嵌套数组
     */
    // return Object.values(highlightedIds).flatMap((s) => [...s])
    // },
  };
}
